...
export function FancyBox({ allowEdit = false }: Props) {
        ...
        <PopoverContent className="w-[200px] p-0">
          <Command loop>
            ...
            <CommandGroup>
              ...
              <CommandItem
                value={`:${inputValue}:delete`} // HACK: that way, the edit button will always be shown
                className="text-xs text-muted-foreground"
                onSelect={() => setSelectedValues([])}
              >
                <div className={cn("mr-2 h-4 w-4")} />
                <Trash className="mr-2 h-2.5 w-2.5" />
                Reset Selection
              </CommandItem>
            </CommandGroup>
          </Command>
